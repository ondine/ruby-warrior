...
Once you are done editing player.rb, save the file and run the "rubywarrior" command again from the rubywarrior-ai directory to start playing the level. The play happens through a series of turns. On each one, your "play_turn" method is called along with any enemy's.
...
Once you pass a level (by reaching the stairs), the profile README will be automagically be updated for the next level. Alter the player.rb file and run rubywarrior again to play the next level.
