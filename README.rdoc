...
You play as a warrior climbing a tall tower to reach the precious Ruby at the top level. On each floor you need to write a Ruby script to instruct the warrior to battle enemies, rescue captives, and reach the stairs. You have some idea of what each floor contains, but you never know for certain what will happen. You must give the Warrior enough artificial intelligence up-front to find her own way.

== Getting Started

First, fork this "ruby-warrior" repository, and then clone it to your machine.
To fork this project, click the "Fork" button (upper right hand corner) in the GitHub.com repository.

  Watch Star Fork [add image file Fork.png here]
  
Congratulations! You've successfully forked the "ruby-warrior" repository, but so far it only exists on your GitHub account as a forked repo. To be able to work on the project, you will need to clone it to your local machine.

To clone your fork, run the following code:

  git clone https://github.com/username/ruby-warrior.git

Install the rubywarrior gem.

  gem install rubywarrior

Then run the "rubywarrior" command to setup your profile. 

  Welcome to Ruby Warrior
    [1] playername - beginner - level 1 - score 0
    [2] playername - intermediate - level 2 - score 0
  Choose profile by typing the number:

This will create a rubywarrior directory in your current location. For example if you chose the beginner level, in the 'playername - beginner' directory you will see two files: A player.rb file and a README file. 
The player.rb file contains the following code:
...
Your objective is to fill this "play_turn" method with commands to instruct the warrior what to do. With each level your abilities will grow along with the difficulty. Now, open the README for details on what abilities your warrior has available on the current level.

Here is a simple example which will instruct the warrior to walk:
...
In this example, the warrior is instructed to attack if she feels an enemy, otherwise the warrior will walk forward.
...
Once you are done editing player.rb, save the file and run the "rubywarrior" command again from the rubywarrior directory to start playing the level. The play happens through a series of turns. On each one, your "play_turn" method is called along with any enemy's.
...
Once you pass a level (by reaching the stairs), the profile README will automagically? be updated for the next level. Alter the player.rb file (how?) and run rubywarrior again to play the next level.

== Perspective

Even though this is a text-based game, think of it as two-dimensional where you are viewing from overhead. Each level is always rectangular in shape and is made up of a number of squares. Only one unit can be on a given square at a time, and === your goal is to find the square with the stairs. ===  Here is an example level map and key.
...
An action is something that effects the game in some way. 
A sense is something which gathers information about the floor.

ACTIONS: You can easily tell an action because it ends in an exclamation mark. Only one action can be performed per turn, so choose wisely. Here are some examples of actions!
...
SENSES: You can perform senses as often as you want per turn to gather information about your surroundings and to aid you in choosing the proper action. Senses do NOT end in an exclamation mark.
...
Your objective is to not only to reach the stairs, but to get the highest score you can. There are many ways you can earn points on a level.
...
* defeat all enemies and rescue all captives to receive a 20 percent overall bonus
...
Remember, you're working in Ruby here. Don't simply fill up the "play_turn" method with a lot of code. Organize it with methods and classes. The player directory is set up as a load path (define load path), which means you can include other ruby files from your player.rb file.
